...
public class LassoFitGenerator {
	...
	public static final long MAX_AVAILABLE_MEMORY = 8L * 1024 * 1024 * 1024;

    /* In order to speed up the compression, we limit the number of 
     * observations, but this limit is dependent on the number of features 
     * that we should learn their weights. In other words, for learning 
     * weights of more features, we need more observations.
     */
    public static final int MAX_OBSERVATIONS_TO_FEATURES_RATIO = 10000;
    ...
    public static final double CONVERGENCE_THRESHOLD = 1.0e-4;

    public static final double SMALL = 1.0e-5;
    public static final int MIN_NUMBER_OF_LAMBDAS = 5;
    public static final double MAX_RSQUARED = 0.99999;
     ...
     */
    public void init(int maxNumFeatures, int numObservations) throws IllegalStateException {
        this.numFeatures = maxNumFeatures;
        if (numObservations > getMaxAllowedObservations(maxNumFeatures)) {
            throw new IllegalStateException("Number of observations (" + numObservations + ") exceeds the maximum allowed number: "
    ...
    public void setFeatureValues(int idx, float[] values) {
        System.arraycopy(values, 0, observations[idx], 0, values.length);
    }
    ...
    private LassoFit getLassoFit(int maxAllowedFeaturesPerModel, int numberOfLambdas) {
		long startTime;
        if (VERBOSE) { 
...
//        	System.out.println("using number of features ("+numFeatures+") instead of "+maxAllowedFeaturesPerModel);
            maxAllowedFeaturesPerModel = numFeatures;
        }
        
        if (numberOfLambdas < MIN_NUMBER_OF_LAMBDAS) {
//        	System.out.println("using minimum number of lambdas ("+MIN_NUMBER_OF_LAMBDAS+") instead of "+numberOfLambdas);
        	numberOfLambdas = MIN_NUMBER_OF_LAMBDAS;
        }
        ...
        for (int j = 0; j < numFeatures; j++) {
            ...
            // control for constants features
            if(featureStds[j]!=0) {
        ...
        double[][] feature2featureCorrelations = MathUtil.allocateDoubleMatrix(numFeatures, maxAllowedFeaturesAlongPath);
        ...
        for (int iteration = 1; iteration <= numberOfLambdas; iteration++) {
        	if (VERBOSE) { System.out.println("Starting iteration " + iteration + " of Compression."); }
            ...
            while (true) {
                ...
                for (int k = 0; k < numFeatures; k++) {
                    ...
                    v = (u >= 0 ? u : -u) - curLambda; // 35X faster than Math.sign(u)(|u| - curLambda)
                    activeWeights[k] = (v > 0 ? (u >= 0 ? v : -v) : 0.0);

                    // skip to next if this variable unchanged
                    if (activeWeights[k] == prevWeight) {
                        continue;
                    }

                    // calculate uncachedCorrelations
                    if (correlationCacheIndices[k] == 0) {
                        numberOfInputs++;
                        if (numberOfInputs > maxAllowedFeaturesAlongPath) {
                            break; // we have reached the maximum
                        }
                        for (int j = 0; j < numFeatures; j++) {
                            if (correlationCacheIndices[j] != 0) {
                            	...
                            	feature2featureCorrelations[j][numberOfInputs-1] = 1.0; // Correlation of variable with itself
                                if (j != k) {
    ...
    /**
     * Update target value for a given index
     * @param idx
     * @param target
     */
    ...
    /**
     * 
     * @param maxAllowedFeaturesPerModel
     * @param numberOfLambdas
     * @return LassoFit
     */
    public LassoFit fit(int maxAllowedFeaturesPerModel, int numberOfLambdas) {
        LassoFit fit = getLassoFit(maxAllowedFeaturesPerModel, numberOfLambdas);
        return fit;
    }
}